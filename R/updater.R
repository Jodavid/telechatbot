...
Mensagens <- dplyr::tibble(DATE_TELEGRAM = c(),
                           DATE_SYSTEM = c(),
                           HOUR = c(),
                           CHAT_ID = c(),
                           IS_BOT = c(),
      ...
      if (length(updates)) {
        for (update in updates) {
          if (private$verbose) {
            ...
            Mensagens <- Mensagens |>
              dplyr::add_row(DATE_TELEGRAM = update$message$date,
                             DATE_SYSTEM = as.character(Sys.Date()),
                             HOUR = as.character(format(Sys.time(), "%X")),
            ...
            cat(sprintf("Processing Update %i with message %s", update$update_id, update$message$text), fill = TRUE)
